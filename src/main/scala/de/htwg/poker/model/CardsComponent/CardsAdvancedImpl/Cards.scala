...
class Card(val suit: Suit, val rank: Rank) extends CardInterface {
  ...
  def CardToHtml: String = {
    s"<div class=\"rounded-lg bg-slate-800 w-6 h-9 hover:scale-125 flex flex-col justify-center items-center shadow-xl shadow-black/50\">${SuitToHtml(suit)}<h1 class=\"font-bold text-slate-100\">${rank.toString}</h1></div>"
  }
