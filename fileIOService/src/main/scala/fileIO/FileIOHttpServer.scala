...
object FileIOHttpServer {
  ...
  def run(
    ...
    val routes = new FileIORoutes().routes
    val host = "0.0.0.0"
    ...
    serverBinding.onComplete {
      case Success(binding) =>
        logger.info(s"FileIO Service running at http://${binding.localAddress.getHostString}:${binding.localAddress.getPort}/")
      case Failure(ex) =>
        logger.error("FileIO Service failed to start", ex)
        system.terminate()
    }
