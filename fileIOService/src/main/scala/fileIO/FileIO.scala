...
class FileIO @Inject(){
  ...
  override def load:GameStateInterface=
	...
	save(gameState: GameStateInterface): Unit = {import java.io._

	val pw = new PrintWriter(new File("gameState.json"))pw.write(Json.prettyPrint(GameStateToJson(gameState)))
    pw.close
  }
	...
	def reverseSuit(suit:String):String= {
		suit match{case"♣"=>"Clubs"case"♠"=>"Spades"case"♢"=>"Diamonds"case"♡"=>"Hearts"}
	}

	def reverseRank(rank:String):String= {
		rank match{case"2"=>"Two"case"3"=>"Three"case"4"=>"Four"case"5"=>"Five"case"6"=>"Six"case"7"=>"Seven"case"8"=>"Eight"case"9"=>"Nine"case"10"=>"Ten"case"J"=>"Jack"case"Q"=>"Queen"case"K"=>"King"case"A"=>"Ace"}
	}

	def reconstructDeck(deck:List[JsValue]):List[Card]= {
		deck.map{card=>val rank=(card\"card"\"rank").as[String]val suit=(card\"card"\"suit").as[String]new Card(Suit.valueOf(reverseSuit(suit)),Rank.valueOf(reverseRank(rank)))}}
}
